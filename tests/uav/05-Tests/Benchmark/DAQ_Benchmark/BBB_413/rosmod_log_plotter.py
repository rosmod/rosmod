...
class ROSMOD_Log_Plotter():
    ...
    def parse(self, log_contents, unit):
        ...
        for line in log_contents.split('\n'):
            ...
            if 'ENQUEUE' in line:
                ...
                new_callback.deadline = float(float(deadline_sec)*1000000000.0 + float(deadline_nsec))/divide
                    ...
                    for cbk in stack:
                        if cbk.name == line.split('::')[4].split(';')[0].split('=')[-1]:
                            if cbk.exec_time == 0.0:
                                ...
                                cbk.exec_time = float(float(exec_time_sec)*1000000000.0 + float(exec_time_nsec))/divide
                                cbk.completion_time = completion_time               
                                break;
        self.callbacks = [call for call in self.callbacks if call.enqueue_time != 0.0 if call.completion_time != 0.0 if call.deadline != 0.0 if call.exec_time != 0.0]
    ...
    def plot(self, opn_max, combined, directory):
        ...
        for call in self.unique_callbacks:
            ...
            plt.xlabel('Test Timestamp (' + self.time_unit + ')')
            plt.ylabel('Execution Time (' + self.time_unit + ')')
            plt.title('Operation Execution Plot - ' + call)
            plt.grid(True)
        ...
        for call in self.unique_callbacks:
            ...
            plt.title(call + " - Execution Times Histogram")
            plt.xlabel("Operation Execution Time (" + self.time_unit + ")")
            plt.ylabel("Number of Operation Instances")
            plt.grid(True)
            plt.legend(loc=1,prop={'size':15})
        fig2 = plt.gcf()
...
def main():
    ...
    parser = argparse.ArgumentParser(description='functionality: Plot Component Operation Execution Behavior', 
    ...
    if args['log'] == 'all':
        ...
        for log in logs:
            if "ROSMOD_DEBUG." in log:
                with open (log, "r") as logs:
                    log_contents += "\nNODE::" + log.split('.')[1] + "::COMPONENT::" + log.split('.')[2] + '\n'
        ...
        with open(args['log'], 'r') as log:
            plotter = ROSMOD_Log_Plotter()
            log_contents = ""
            log_contents += "\nNODE::" + args['log'].split('.')[1] + "::COMPONENT::" + args['log'].split('.')[2] + '\n'
