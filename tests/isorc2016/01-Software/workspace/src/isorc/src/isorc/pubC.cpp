...
//# Start Init Marker
void pubC::init_timer_operation(const NAMESPACE::TimerEvent& event)
{
#ifdef USE_ROSMOD
  comp_queue.ROSMOD_LOGGER->log("DEBUG", "Entering pubC::init_timer_operation");
#endif
  // Initialize Here
  ...
  if (config.profileMap.find("c_pub") != config.profileMap.end())
    {
      c_pub_send_mw.init(node_argc,
					     node_argv,
					     config.profileMap["c_pub"],
					     config.uuidMap["c_pub"],
					     enable_oob);
      if ( tg_duration < 0 )
	c_pub_send_mw.set_duration(c_pub_send_mw.profile.period);
      else
	c_pub_send_mw.set_duration(tg_duration);
      fName = "/var/log/" + config.nodeName + "." + config.compName + ".c_pub.network.csv";
      c_pub_send_mw.set_output_filename(fName);
    }
  // Stop Init Timer
  init_timer.stop();
#ifdef USE_ROSMOD
  comp_queue.ROSMOD_LOGGER->log("DEBUG", "Exiting pubC::init_timer_operation");
#endif  
}
//# End Init Marker
...
{
#ifdef USE_ROSMOD
  comp_queue.ROSMOD_LOGGER->log("DEBUG", "Entering pubC::pub_timer_operation");
#endif
  ...
  double offset = c_pub_send_mw.profile.getOffset(current_time);
  double period = c_pub_send_mw.profile.period;
...
#ifdef USE_ROSMOD
  comp_queue.ROSMOD_LOGGER->log("DEBUG", "Exiting pubC::pub_timer_operation");
#endif
}
...
void pubC::startUp()
{
  ...
  // Component Publisher - c_pub
  advertiseName = "messageC";
  if (config.portGroupMap.find("c_pub") != config.portGroupMap.end())
    advertiseName += "_" + config.portGroupMap["c_pub"];
  this->c_pub = nh.advertise<isorc::messageC>(advertiseName.c_str(), 1000);
  ...
  if (config.num_comps_to_sync > 1 )
	...
	("component_synchronization",
	 1000,
	 boost::bind(&pubC::component_sync_operation, this, _1),
	 NAMESPACE::VoidPtr(),
#ifdef USE_ROSMOD     
	 &this->comp_queue,
  ...
  timer_options = 
    NAMESPACE::TimerOptions
    (ros::Duration(-1),
     boost::bind(&pubC::init_timer_operation, this, _1),
     &this->comp_queue,
#ifdef USE_ROSMOD     
     callback_options,
#endif     
  ...
  timer_options = 
    NAMESPACE::TimerOptions
    (ros::Duration(0.05),
     boost::bind(&pubC::pub_timer_operation, this, _1),
     &this->comp_queue,
#ifdef USE_ROSMOD     
     callback_options,
#endif 
...
extern "C" {
  Component *maker(ComponentConfig &config, int argc, char **argv) {
    return new pubC(config,argc,argv);
  }
}
