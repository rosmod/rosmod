...
//# Start Init Marker
void pubD::init_timer_operation(const NAMESPACE::TimerEvent& event)
{
#ifdef USE_ROSMOD
  ...
  if (config.profileMap.find("d_pub") != config.profileMap.end())
    {
      d_pub_send_mw.init(node_argc,
					     node_argv,
					     config.profileMap["d_pub"],
					     config.uuidMap["d_pub"],
					     enable_oob);
      if ( tg_duration < 0 )
	d_pub_send_mw.set_duration(d_pub_send_mw.profile.period);
      else
	d_pub_send_mw.set_duration(tg_duration);
      fName = "/var/log/" + config.nodeName + "." + config.compName + ".d_pub.network.csv";
      d_pub_send_mw.set_output_filename(fName);
    }
  // Stop Init Timer
  init_timer.stop();
#ifdef USE_ROSMOD
  comp_queue.ROSMOD_LOGGER->log("DEBUG", "Exiting pubD::init_timer_operation");
#endif  
}
//# End Init Marker
...
void pubD::d_pub_timer_operation(const NAMESPACE::TimerEvent& event)
{
  ...
  try
    {
      timerDelay =
	d_pub_send_mw.send<isorc::messageD>(d_pub, msg);
    }
  ...
  if ( ros::Time::now() >= d_pub_send_mw.get_end_time() )
    {
...
void pubD::startUp()
{
  ...
  // Component Publisher - d_pub
  advertiseName = "messageD";
  if (config.portGroupMap.find("d_pub") != config.portGroupMap.end())
    advertiseName += "_" + config.portGroupMap["d_pub"];
  this->d_pub = nh.advertise<isorc::messageD>(advertiseName.c_str(), 1000);
  ...
  if (config.num_comps_to_sync > 1 )
	...
	("component_synchronization",
	 1000,
	 boost::bind(&pubD::component_sync_operation, this, _1),
	 NAMESPACE::VoidPtr(),
#ifdef USE_ROSMOD     
	 &this->comp_queue,
  ...
  timer_options = 
    NAMESPACE::TimerOptions
    (ros::Duration(-1),
     boost::bind(&pubD::init_timer_operation, this, _1),
     &this->comp_queue,
#ifdef USE_ROSMOD     
     callback_options,
#endif     
  ...
  timer_options = 
    NAMESPACE::TimerOptions
    (ros::Duration(0.05),
     boost::bind(&pubD::pub_timer_operation, this, _1),
     &this->comp_queue,
#ifdef USE_ROSMOD     
     callback_options,
#endif 
...
extern "C" {
  Component *maker(ComponentConfig &config, int argc, char **argv) {
    return new pubD(config,argc,argv);
  }
}
