...
//# Start Init Marker
void pubA::init_timer_operation(const NAMESPACE::TimerEvent& event)
{
#ifdef USE_ROSMOD
  comp_queue.ROSMOD_LOGGER->log("DEBUG", "Entering pubA::init_timer_operation");
#endif
  // Initialize Here
  ...
  if (config.profileMap.find("a_pub") != config.profileMap.end())
    {
      a_pub_send_mw.init(node_argc,
					     node_argv,
					     config.profileMap["a_pub"],
					     config.uuidMap["a_pub"],
					     enable_oob);
      if ( tg_duration < 0 )
  ...
  init_timer.stop();
#ifdef USE_ROSMOD
  comp_queue.ROSMOD_LOGGER->log("DEBUG", "Exiting pubA::init_timer_operation");
#endif  
}
//# End Init Marker

void pubA::a_pub_timer_operation(const NAMESPACE::TimerEvent& event)
{
  isorc::messageA msg;
  msg.uuid = a_pub_send_mw.get_uuid();
  msg.bytes.resize(max_data_length,0);
  double timerDelay = 0;
  try
    {
      timerDelay =
	a_pub_send_mw.send<isorc::messageA>(a_pub, msg);
    }
  ...
  if ( ros::Time::now() >= a_pub_send_mw.get_end_time() )
    {
      logger->log("DEBUG","writing output\n");
      a_pub_send_mw.record();
    }
  else
    {
      if (tg_misbehave)
	...
	(ros::Duration(timerDelay),
	 boost::bind(&pubA::a_pub_timer_operation, this, _1),
	 &this->comp_queue,
#ifdef USE_ROSMOD     
	 callback_options,
#endif 
	 true);
      ...
      a_pub_timer = nh.createTimer(timer_options);
    }
}

// Timer Callback - pub_timer
//# Start pub_timer_operation Marker
void pubA::pub_timer_operation(const NAMESPACE::TimerEvent& event)
{
#ifdef USE_ROSMOD
  comp_queue.ROSMOD_LOGGER->log("DEBUG", "Entering pubA::pub_timer_operation");
#endif
  ...
  double offset = a_pub_send_mw.profile.getOffset(current_time);
  double period = a_pub_send_mw.profile.period;
...
#ifdef USE_ROSMOD
  comp_queue.ROSMOD_LOGGER->log("DEBUG", "Exiting pubA::pub_timer_operation");
#endif
}
...
void pubA::startUp()
{
  ...
  // Component Publisher - a_pub
  advertiseName = "messageA";
  if (config.portGroupMap.find("a_pub") != config.portGroupMap.end())
    advertiseName += "_" + config.portGroupMap["a_pub"];
  this->a_pub = nh.advertise<isorc::messageA>(advertiseName.c_str(), 1000);
  ...
  if (config.num_comps_to_sync > 1 )
	...
	("component_synchronization",
	 1000,
	 boost::bind(&pubA::component_sync_operation, this, _1),
	 NAMESPACE::VoidPtr(),
#ifdef USE_ROSMOD     
	 &this->comp_queue,
  ...
  timer_options = 
    NAMESPACE::TimerOptions
    (ros::Duration(-1),
     boost::bind(&pubA::init_timer_operation, this, _1),
     &this->comp_queue,
#ifdef USE_ROSMOD     
     callback_options,
#endif     
  ...
  timer_options = 
    NAMESPACE::TimerOptions
    (ros::Duration(0.05),
     boost::bind(&pubA::pub_timer_operation, this, _1),
     &this->comp_queue,
#ifdef USE_ROSMOD     
     callback_options,
#endif 
...
extern "C" {
  Component *maker(ComponentConfig &config, int argc, char **argv) {
    return new pubA(config,argc,argv);
  }
}
