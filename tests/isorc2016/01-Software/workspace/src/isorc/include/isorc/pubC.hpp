...
class pubC : public Component
{
public:
  // Constructor
  pubC(ComponentConfig& _config, int argc, char **argv)
...
private:
  ...
  // Publisher 
  NAMESPACE::Publisher c_pub;
  // Timer for generating traffic
  NAMESPACE::Timer c_pub_timer;
  // Timer operation for traffic generation
  void c_pub_timer_operation(const NAMESPACE::TimerEvent& event);
  // publisher sender middleware
  Network::sender c_pub_send_mw;
