...
void tl_sensor::ryg_state_pub_timer_operation(const NAMESPACE::TimerEvent& event)
{
  tlc::ryg_state msg;
  ...
  try
    {
      timerDelay =
	ryg_state_pub_send_mw.send<tlc::ryg_state>(ryg_state_pub, msg);
    }
  ...
  if ( ros::Time::now() >= ryg_state_pub_send_mw.get_end_time() )
	...
	(ros::Duration(timerDelay),
	 boost::bind(&tl_sensor::ryg_state_pub_timer_operation, this, _1),
	 &this->comp_queue,
#ifdef USE_ROSMOD     
	 callback_options,
#endif 
