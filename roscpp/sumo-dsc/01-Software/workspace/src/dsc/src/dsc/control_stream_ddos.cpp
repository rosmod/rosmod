#include "dsc/control_stream_ddos.hpp"
...
//# Start Init Marker
void control_stream_ddos::Init(const ros::TimerEvent& event)
{
  // Initialize Here
  srand (time(NULL));
  ...
  if (config.profileMap.find("control_stream_ddos_pub") != config.profileMap.end())
	...
	control_stream_ddos_pub_send_mw.set_duration(tg_duration);
      fName = nodeName + "." + compName + ".control_stream_ddos_pub.network.csv";
      control_stream_ddos_pub_send_mw.set_output_filename(fName);

      timer_options = 
	ros::TimerOptions
	(ros::Duration(-1),
	 boost::bind(&control_stream_ddos::control_stream_ddos_pub_timerCallback, this, _1),
	 &this->compQueue,
	 true);
...
void control_stream_ddos::control_stream_ddos_pub_timerCallback(const ros::TimerEvent& event)
{
  dsc::ryg_control msg;
  msg.intersection_name.resize(max_data_length,'x');
  ...
  try
    {
      timerDelay =
	control_stream_ddos_pub_send_mw.send<dsc::ryg_control>(control_stream_ddos_pub, msg);
    }
...
control_stream_ddos::~control_stream_ddos()
{
  control_stream_ddos_pub.shutdown();
  //# Start Destructor Marker
  //# End Destructor Marker
}

// Startup - Setup Component Ports & Timers
void control_stream_ddos::startUp()
{
  ros::NodeHandle nh;
  ...
  // Component Publisher - control_stream_ddos_pub
  advertiseName = "ryg_control";
  if (portGroupMap.find("control_stream_ddos_pub") != portGroupMap.end())
    advertiseName += "_" + portGroupMap["control_stream_ddos_pub"];
  this->control_stream_ddos_pub = nh.advertise<dsc::ryg_control>(advertiseName.c_str(), 1000);
  ...
  timer_options = 
    ros::TimerOptions
    (ros::Duration(-1),
     boost::bind(&control_stream_ddos::Init, this, _1),
     &this->compQueue,
     true,
     false);
  ...
  comp_sync_sub_options = ros::SubscribeOptions::create<std_msgs::Bool>
    ("component_synchronization",
     1000,
     boost::bind(&control_stream_ddos::component_synchronization_OnOneData, this, _1),
     ros::VoidPtr(),
     &this->compQueue);
...
extern "C" {
  Component *maker(ComponentConfig &config, int argc, char **argv) {
    return new control_stream_ddos(config,argc,argv);
  }
}
