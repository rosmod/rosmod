...
then
    ...
    $TC class add dev ${DEV} parent 2: classid 2:1 htb rate 100Kbit ceil 100Kbit
    $TC class add dev ${DEV} parent 2:1 classid 2:10 htb rate 100Kbit ceil 100Kbit
...
$TC filter add dev ${DEV} protocol ip parent 1: prio 1 u32 \
    match ip dst 10.1.1.15 flowid 1:1
$TC filter add dev ${DEV} protocol ip parent 1: prio 1 u32 \
    match ip dst 10.1.1.16 flowid 1:1
...
then
	...
	match ip dst 10.1.1.15 flowid 2:10

    $TC filter add dev ${DEV} protocol ip parent 2: prio 1 u32 \
	match ip dst 10.1.1.16 flowid 2:10
fi
