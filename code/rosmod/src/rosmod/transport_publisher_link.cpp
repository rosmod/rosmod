...
TransportPublisherLink2::TransportPublisherLink2(const SubscriptionPtr2& parent, const std::string& xmlrpc_uri, const TransportHints& transport_hints)
...
TransportPublisherLink2::~TransportPublisherLink2()
{
  dropping_ = true;

  if (retry_timer_handle_ != -1)
  {
    getInternalTimerManager2()->remove(retry_timer_handle_);
  }

  connection_->drop(Connection::Destructing);
}

bool TransportPublisherLink2::initialize(const ConnectionPtr& connection)
{
  connection_ = connection;
  connection_->addDropListener(boost::bind(&TransportPublisherLink2::onConnectionDropped, this, _1, _2));

  if (connection_->getTransport()->requiresHeader())
  {
    connection_->setHeaderReceivedCallback(boost::bind(&TransportPublisherLink2::onHeaderReceived, this, _1, _2));
    ...
    connection_->writeHeader(header, boost::bind(&TransportPublisherLink2::onHeaderWritten, this, _1));
  }
  else
  {
    connection_->read(4, boost::bind(&TransportPublisherLink2::onMessageLength, this, _1, _2, _3, _4));
  }

  return true;
}

void TransportPublisherLink2::drop()
{
  dropping_ = true;
  connection_->drop(Connection::Destructing);
...
void TransportPublisherLink2::onHeaderWritten(const ConnectionPtr& conn)
{
  // Do nothing
}

bool TransportPublisherLink2::onHeaderReceived(const ConnectionPtr& conn, const Header& header)
{
...
void TransportPublisherLink2::onMessageLength(const ConnectionPtr& conn, const boost::shared_array<uint8_t>& buffer, uint32_t size, bool success)
{
  if (retry_timer_handle_ != -1)
  {
    getInternalTimerManager2()->remove(retry_timer_handle_);
  ...
  connection_->read(len, boost::bind(&TransportPublisherLink2::onMessage, this, _1, _2, _3, _4));
}

void TransportPublisherLink2::onMessage(const ConnectionPtr& conn, const boost::shared_array<uint8_t>& buffer, uint32_t size, bool success)
{
  if (!success && !conn)
    return;
...
void TransportPublisherLink2::onRetryTimer(const ros::WallTimerEvent&)
{
  if (dropping_)
  {
    return;
...
CMQPtr getInternalCallbackQueue();

void TransportPublisherLink2::onConnectionDropped(const ConnectionPtr& conn, Connection::DropReason reason)
{
  if (dropping_)
  {
    return;
  ...
  if (reason == Connection::TransportDisconnect)
  {
    ...
    if (retry_timer_handle_ == -1)
    {
      ...
      next_retry_ = WallTime::now() + retry_period_;
      retry_timer_handle_ = getInternalTimerManager2()->add(WallDuration(retry_period_),
          boost::bind(&TransportPublisherLink2::onRetryTimer, this, _1), getInternalCallbackQueue().get(),          parent->priority_, VoidConstPtr(), false);
    }
    else
    {
      getInternalTimerManager2()->setPeriod(retry_timer_handle_, retry_period_);
    }
  }
  else
  {
...
void TransportPublisherLink2::handleMessage(const SerializedMessage& m, bool ser, bool nocopy)
{
...
std::string TransportPublisherLink2::getTransportType()
{
  return connection_->getTransport()->getType();
}

std::string TransportPublisherLink2::getTransportInfo()
{
