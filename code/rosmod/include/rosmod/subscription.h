#ifndef ROSMOD_SUBSCRIPTION_H
#define ROSMOD_SUBSCRIPTION_H
...
class PublisherLink2;
typedef boost::shared_ptr<PublisherLink2> PublisherLinkPtr2;
 ...
 */
class Subscription2 : public boost::enable_shared_from_this<Subscription2>
{
public:
  Subscription2(const std::string &name, const std::string& md5sum, const std::string& datatype, const TransportHints& transport_hints);
  virtual ~Subscription2();
  ...
  void addLocalConnection(const PublicationPtr2& pub);
  ...
  bool addCallback(const SubscriptionCallbackHelperPtr& helper, const std::string& md5sum, CMQ* queue, int priority, int32_t queue_size, const VoidConstPtr& tracked_object, bool allow_concurrent_callbacks);
   ...
   */
  uint32_t handleMessage(const SerializedMessage& m, bool ser, bool nocopy, const boost::shared_ptr<M_string>& connection_header, const PublisherLinkPtr2& link);
   ...
   */
  void removePublisherLink(const PublisherLinkPtr2& pub_link);
  ...
  {
    public:
      PendingConnection(XmlRpc::XmlRpcClient* client, TransportUDPPtr udp_transport, const SubscriptionWPtr2& parent, const std::string& remote_uri)
      : client_(client)
  ...
  void headerReceived(const PublisherLinkPtr2& link, const Header& h);
...
private:
  Subscription2(const Subscription2 &); // not copyable
  Subscription2 &operator =(const Subscription2 &); // nor assignable

  void dropAllConnections();

  void addPublisherLink(const PublisherLinkPtr2& link);
  ...
  typedef std::vector<PublisherLinkPtr2> V_PublisherLink2;
  V_PublisherLink2 publisher_links_;
  ...
  struct LatchInfo
  {
    ...
    PublisherLinkPtr2 link;
    boost::shared_ptr<std::map<std::string, std::string> > connection_header;
    ros::Time receipt_time;
  };

  typedef std::map<PublisherLinkPtr2, LatchInfo> M_PublisherLinkToLatchInfo;
