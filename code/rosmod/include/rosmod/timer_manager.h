#ifndef ROSMOD_TIMER_MANAGER_H
#define ROSMOD_TIMER_MANAGER_H
...
public:
  ...
  int32_t add(const D& period, const boost::function<void(const E&)>& callback, CMQ* callback_queue, int priority, const VoidConstPtr& tracked_object, bool oneshot);
  void remove(int32_t handle);

  bool hasPending(int32_t handle);
  void setPeriod(int32_t handle, const D& period);

  static TimerManager2& global()
  {
    static TimerManager2<T, D, E> global;
    return global;
  }
...
private:
  ...
  {
  public:
    TimerQueueCallback(TimerManager2<T, D, E>* parent, const TimerInfoPtr& info, T last_expected, T last_real, T current_expected)
    : parent_(parent)
    , info_(info)
...
template<class T, class D, class E>
TimerManager2<T, D, E>::TimerManager2() :
...
template<class T, class D, class E>
bool TimerManager2<T, D, E>::waitingCompare(int32_t lhs, int32_t rhs)
{
...
template<class T, class D, class E>
typename TimerManager2<T, D, E>::TimerInfoPtr TimerManager2<T, D, E>::findTimer(int32_t handle)
{
...
template<class T, class D, class E>
bool TimerManager2<T, D, E>::hasPending(int32_t handle)
{
  boost::mutex::scoped_lock lock(timers_mutex_);
...
template<class T, class D, class E>
int32_t TimerManager2<T, D, E>::add(const D& period, const boost::function<void(const E&)>& callback, CMQ* callback_queue, int priority,
                                   const VoidConstPtr& tracked_object, bool oneshot)
{
...

  {
    ...
    if (!thread_started_)
    {
      thread_ = boost::thread(boost::bind(&TimerManager2::threadFunc, this));
...

    {
      ...
      waiting_.sort(boost::bind(&TimerManager2::waitingCompare, this, _1, _2));
    }
...
template<class T, class D, class E>
void TimerManager2<T, D, E>::remove(int32_t handle)
{
  CMQ* callback_queue = 0;
...
template<class T, class D, class E>
void TimerManager2<T, D, E>::schedule(const TimerInfoPtr& info)
{
  boost::mutex::scoped_lock lock(timers_mutex_);
...
template<class T, class D, class E>
void TimerManager2<T, D, E>::updateNext(const TimerInfoPtr& info, const T& current_time)
{
  if (info->oneshot)
  {
...
template<class T, class D, class E>
void TimerManager2<T, D, E>::setPeriod(int32_t handle, const D& period)
{
  boost::mutex::scoped_lock lock(timers_mutex_);
  TimerInfoPtr info = findTimer(handle);
...
template<class T, class D, class E>
void TimerManager2<T, D, E>::threadFunc()
{
  T current;
  while (!quit_)
  {
...

    {
      ...
      else
      {
        ...
        while (!waiting_.empty() && info && info->next_expected <= current)
        {
          ...
          info->callback_queue->addCallback(cb, info->priority, (uint64_t)info.get());
