...
public:
  ...
  bool Parse(std::string fName)
  {
    ...
    for (xml_node<> *comp_inst = node->first_node("component_instance"); comp_inst; comp_inst = comp_inst->next_sibling())
      {
	...
	config.logLevels.DEBUG = Return_Boolean(logger->first_node("debug")->first_attribute()->value());
	config.logLevels.INFO = Return_Boolean(logger->first_node("info")->first_attribute()->value());
	config.logLevels.WARNING = Return_Boolean(logger->first_node("warning")->first_attribute()->value());
	config.logLevels.ERROR = Return_Boolean(logger->first_node("error")->first_attribute()->value());
	config.logLevels.CRITICAL = Return_Boolean(logger->first_node("critical")->first_attribute()->value());

	for (xml_node<> *port_inst = comp_inst->first_node("port_instance"); port_inst; port_inst = port_inst->next_sibling())
	  {
